...
class AuditLogService {
  ...
  async getAuditLogs(filters?: AuditLogFilters): Promise<AuditLog[]> {
    try {
      ...
      const userId = await this.getUserIdOrNull();
      if (!userId) {
        return [];
      }
  ...
  private async addToBuffer(log: AuditLog): Promise<void> {
    const userId = await this.getUserIdOrNull();
    if (!userId) {
      console.warn('Skipping audit log buffer write: user not authenticated');
      return;
    }
    if (!log.user_id) {
      log.user_id = userId;
  ...
  private async getUserIdOrNull(): Promise<string | null> {
    try {
